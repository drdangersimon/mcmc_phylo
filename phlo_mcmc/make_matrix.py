...
def call_laplace(cmd, run_file):
    """
    Calls the laplace code to calculate the likelihood.
    :param cmd: str path to laplace function
    :param run_file: Path to parameters in a matrix format for reading by laplace
    :return: float
    """
...
def generate_inital_matrix(row=5, col=5):
    ...
    are == 1
    :param row: int number of rows in matrix
    :param col: int number of columns in matrix"""
    values = [0, 0.5, 1]
    mat = []
    for i in range(row):
...
def chng_matrix_item(mat, index, values=[0, 0.5, 1]):
    ...
    :param mat: ndarray parameter matrix
    :param index: int
    :param values: list"""
    new_mat = nu.copy(mat)
...
def write_matrix(mat, filename):
    ...
    :param mat: list matrix to write
    :param filename: path file to write matrix"""
...
##########################################################
# Everythng below here is deprecated
##########################################################
